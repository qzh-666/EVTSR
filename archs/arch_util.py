...
def resize_flow(flow, size_type, sizes, interp_mode=bilinear, align_corners=False):
    ...
    if size_type == ratio:
        ...
        raise ValueError(fSize type should be ratio or shape, but got type {size_type}.)
...
class DCNv2Pack(ModulatedDeformConvPack):
    ...
    def forward(self, x, feat):
        ...
        if offset_absmean > 50:
            ...
            logger.warning(fOffset abs mean is {offset_absmean}, larger than 50.)
...
def _no_grad_trunc_normal_(tensor, mean, std, a, b):
    ...
    # Cut & paste from PyTorch official master until its in a few official releases - RW
    ...
    with torch.no_grad():
        ...
        # Clamp to ensure its in the proper range
        tensor.clamp_(min=a, max=b)
        return tensor


def trunc_normal_(tensor, mean=0., std=1., a=-2., b=2.):
    rFills the input Tensor with values drawn from a truncated
    normal distribution.
...
class SizeAdapter(object):
    ...
    and unpadding the networks output to the original size.
